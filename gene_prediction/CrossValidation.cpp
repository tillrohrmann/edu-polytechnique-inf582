...
void CrossValidation::modelLearning(const std::string& prefix, boost::shared_ptr<HMM> hmm, const std::vector<DatabaseEntry*>& entries,
		int testsetSize, int tries, const std::string& errorEvaluation, double threshold, bool annotated){
	...
	for(int start=0, i=0; start < numberSequences; start+=testsetSize,i++){
		...
		testset.clear();
		annotations.clear();
		GeneDatabase::separateSet(entries,sequences,testset,annotations,annotated,start,end);
		...
		for(int j=0; j< tries; j++){
			if(errorEvaluation == "Threshold"){
				...
				analyticsResult[i][j] = chmms[i][j]->baumWelch(hmm,substitution,
						inverseSubstitution,sequences,testset,annotations,threshold,annotated);
				matchingScore[i][j] = Analytics::evaluate(analyticsResult[i][j]);

			}else if(errorEvaluation == "Iteration"){
				analyticsResult[i][j] = chmms[i][j]->baumWelchIterated(hmm,substitution,
						inverseSubstitution,sequences,testset,annotations,(int)threshold,annotated);
				matchingScore[i][j] = Analytics::evaluate(analyticsResult[i][j]);
			}
		}
